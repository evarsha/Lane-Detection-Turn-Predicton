...
# slding window concept and plotting the fitted lines 
def extract_plot_lane_line(histogram, input_image ,plot=True):
        ...
        return  new_img,left_line, right_line, ploty, left_linex, right_linex, left_x, left_y, right_x, right_y, left_lane_pixels, right_lane_pixels
...
# plotting back the values in real world 
...
while (True):
    ret,image=c.read()
    if ret == True:   
        undistort=cv2.undistort(image,K,D,None,K)
        ...
        sobel = np.sqrt((sobelx**2) + (sobely**2)) # sobe l
        histogram = np.sum(sobel[sobel.shape[0]//2:,:], axis=0)
        new_img,left_line, right_line, ploty, left_linex, right_linex, left_x, left_y, right_x, right_y, left_lane_pixels, right_lane_pixels=extract_plot_lane_line(histogram, sobel)
